...
router.get('/export', auth_1.authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const buffer = await (0, excel_1.createInfrastrukturExcel)(data);
...
router.post('/', auth_1.authMiddleware, async (req, res) => {
    ...
    if (isNaN(latNum) || latNum < -90 || latNum > 90) {
        res.status(400).json({ error: 'Latitude tidak valid' });
        return;
    }
    if (isNaN(lngNum) || lngNum < -180 || lngNum > 180) {
        res.status(400).json({ error: 'Longitude tidak valid' });
        return;
    }
    try {
...
router.post('/import', auth_1.authMiddleware, upload_1.upload.single('file'), async (req, res) => {
    if (!req.file) {
    ...
    try {
        const rows = await (0, excel_1.readExcelFile)(filePath);
        if (rows.length > constants_1.MAX_IMPORT_ROWS) {
            res.status(400).json({ error: `Jumlah baris melebihi batas maksimum ${constants_1.MAX_IMPORT_ROWS} baris` });
            return;
        }
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                ...
                if (!nama) {
                    errors.push({ baris: nomorBaris, pesan: 'Kolom nama tidak boleh kosong' });
                    continue;
                }
                if (!kategori || !kategoriSet.has(kategori)) {
                    errors.push({ baris: nomorBaris, pesan: `Kategori "${kategori}" tidak valid` });
                    continue;
                }
                if (isNaN(lat) || lat < -90 || lat > 90) {
                    errors.push({ baris: nomorBaris, pesan: 'Latitude tidak valid' });
                    continue;
                }
                if (isNaN(lng) || lng < -180 || lng > 180) {
                    errors.push({ baris: nomorBaris, pesan: 'Longitude tidak valid' });
                    continue;
                }
                if (idkab !== constants_1.IDKAB_PADANG_PARIAMAN) {
                    errors.push({ baris: nomorBaris, pesan: `idkab harus "${constants_1.IDKAB_PADANG_PARIAMAN}"` });
                    continue;
                }
                if (idkec.length !== 6 || !idkec.startsWith(constants_1.IDKAB_PADANG_PARIAMAN)) {
                    errors.push({ baris: nomorBaris, pesan: 'idkec tidak valid (harus 6 digit, dimulai dengan 1306)' });
                    continue;
                }
                if (iddesa.length !== 10 || !iddesa.startsWith(idkec)) {
                    errors.push({ baris: nomorBaris, pesan: 'iddesa tidak valid (harus 10 digit)' });
                    continue;
                }
                if (idsls && (idsls.length !== 12 || !idsls.startsWith(iddesa))) {
                    errors.push({ baris: nomorBaris, pesan: 'idsls tidak valid (harus 12 digit)' });
                    continue;
                }
                if (fotoUrl && !fotoUrl.startsWith('http://') && !fotoUrl.startsWith('https://') && !fotoUrl.startsWith('/uploads/')) {
                    errors.push({ baris: nomorBaris, pesan: 'foto_url harus berupa URL valid' });
                    continue;
                }
        ...
        (0, excel_1.cleanupFile)(filePath);
    }
});
...
//# sourceMappingURL=infrastruktur.js.map