...
function getLevelStatistik(row) {
    if (row.idsls)
        return 3;
    if (row.iddesa)
        return 2;
...
function getModeAgregasi(indikator, satuan) {
    const teks = `${indikator} ${satuan ?? ''}`.toLowerCase();
    const isRerata = ['ipm', 'indeks', 'persen', 'persentase', 'rasio', 'tingkat', '%'].some((kata) => teks.includes(kata));
    return isRerata ? 'avg' : 'sum';
}
function agregasiStatistik(rows) {
...
router.get('/indikator', async (_req, res) => {
    try {
...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        if (indikator)
            where.indikator = { contains: String(indikator), mode: 'insensitive' };
        if (aggregate === 'true') {
            if (idsls) {
            ...
            const data = agregasiStatistik(rows);
...
router.get('/export', auth_1.authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const buffer = await (0, excel_1.createStatistikExcel)(data);
...
router.post('/', auth_1.authMiddleware, async (req, res) => {
    ...
    if (!idkab || !indikator || nilai === undefined || !tahun) {
        res.status(400).json({ error: 'Field idkab, indikator, nilai, dan tahun wajib diisi' });
        return;
    }
    const nilaiNum = parseFloat(nilai);
    const tahunNum = parseInt(tahun);
    if (isNaN(nilaiNum)) {
        res.status(400).json({ error: 'Field nilai harus berupa angka' });
        return;
    }
    if (isNaN(tahunNum) || tahunNum < 2000 || tahunNum > 2100) {
        res.status(400).json({ error: 'Field tahun tidak valid' });
        return;
    }
    try {
...
router.post('/import', auth_1.authMiddleware, upload_1.upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        if (rows.length > constants_1.MAX_IMPORT_ROWS) {
            res.status(400).json({ error: `Jumlah baris melebihi batas maksimum ${constants_1.MAX_IMPORT_ROWS}` });
            return;
        }
        let berhasil = 0;
        const errors = [];
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                ...
                if (!idkab) {
                    errors.push({ baris: nomorBaris, pesan: 'idkab tidak boleh kosong' });
                    continue;
                }
                if (!indikator) {
                    errors.push({ baris: nomorBaris, pesan: 'indikator tidak boleh kosong' });
                    continue;
                }
                if (isNaN(nilai)) {
                    errors.push({ baris: nomorBaris, pesan: 'nilai harus berupa angka' });
                    continue;
                }
                if (isNaN(tahun)) {
                    errors.push({ baris: nomorBaris, pesan: 'tahun tidak valid' });
                    continue;
                }
...
router.put('/:id', auth_1.authMiddleware, async (req, res) => {
    ...
    const tahunNum = parseInt(tahun);
    if (isNaN(nilaiNum)) {
        res.status(400).json({ error: 'Nilai harus berupa angka' });
        return;
    }
    if (isNaN(tahunNum)) {
...
//# sourceMappingURL=statistik.js.map