...
router.get('/kecamatan', async (req, res) => {
    ...
    if (!idkab) {
        res.status(400).json({ error: 'Parameter idkab wajib diisi' });
        return;
    }
    try {
...
router.get('/nagari', async (req, res) => {
    const { idkec } = req.query;
    if (!idkec) {
        res.status(400).json({ error: 'Parameter idkec wajib diisi' });
        return;
    }
    try {
        const fromInfra = await prisma.infrastruktur.findMany({
...
router.get('/korong', async (req, res) => {
    const { iddesa } = req.query;
    if (!iddesa) {
        res.status(400).json({ error: 'Parameter iddesa wajib diisi' });
        return;
    }
    try {
